...
Latest commit e1ad687 16 hours ago
 History
 1 contributor
...
# giới hạn thiết bị
  read -p " Nhập tên miền sub miền:" domain
  [ -z "${domain}" ] && domain=0
  echo "-------------------------------"
  echo -e "sub miền là: ${domain}"
  echo "-------------------------------"

  read -p " nhập web có cả https or http:" web
  [ -z "${web}" ] && web=0
  echo "-------------------------------"
  echo -e "đã cập nhật trong file: ${web}"
  echo "-------------------------------"

    read -p " khoá giao tiếp:" api
  [ -z "${api}" ] && api=0
  echo "-------------------------------"
  echo -e "đã cập nhật khoá giao tiếp: ${api}"
  echo "-------------------------------"
